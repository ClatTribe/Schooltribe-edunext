...
export default function TestsPage() {
  ...
  const subjects = profile?.subjects || ['science', 'maths'];
  ...
  if (view === 'test') {
      ...
      return (
        <div className="space-y-8 pb-20 lg:pb-0">
          <header className="space-y-2">
            <button onClick={() => setView('chapters')} className="text-sm font-medium text-amber-400 hover:text-amber-300 transition-colors flex items-center gap-1">
              ← Back to Chapters
            </button>
            <h1 className="text-3xl font-bold text-white">{selectedChapter}</h1>
          </header>
          <div className="glass-card flex flex-col items-center justify-center py-20 space-y-4">
            <div className="h-10 w-10 animate-spin rounded-full border-3 border-surface-700 border-t-amber-400" />
            <p className="text-slate-400 text-lg">Generating questions with AI...</p>
            <p className="text-slate-500 text-sm">This may take a few seconds the first time</p>
          </div>
        </div>
      );
    }
    ...
    if (error || questions.length === 0) {
      return (
        <div className="space-y-8 pb-20 lg:pb-0">
          ...
          <div className="glass-card flex flex-col items-center justify-center py-16 space-y-4">
            <p className="text-3xl">😔</p>
            <p className="text-slate-300 text-lg font-medium">Couldn't load questions</p>
            <p className="text-slate-500 text-sm max-w-md text-center">{error || 'No questions available.'}</p>
    ...
    return (
      <div className="space-y-8 pb-20 lg:pb-0">
        ...
        {/* Score Card */}
        {submitted && (
          <div className="glass-card rounded-2xl p-8 border border-surface-700 bg-gradient-to-br from-amber-900/20 to-transparent">
            <div className="text-center space-y-4">
              <div className="space-y-2">
                <p className="text-5xl font-bold text-gradient">{score}/{questions.length}</p>
                <div className="h-1 w-24 bg-gradient-to-r from-amber-400 to-amber-600 rounded-full mx-auto" />
              </div>
              <p className="text-lg font-semibold text-white">
                {score === questions.length ? '🎉 Perfect Score!' : score >= questions.length * 0.7 ? '👍 Great Job!' : '💪 Keep Practicing'}
              </p>
              <p className="text-slate-400 text-sm">{score} out of {questions.length} correct</p>
              {xpEarned > 0 && (
                <div className="inline-flex items-center gap-2 rounded-full bg-amber-500/20 px-4 py-2 border border-amber-500/30">
                  <span className="text-amber-400 font-bold">+{xpEarned} XP earned!</span>
                </div>
              )}
              {saving && <p className="text-xs text-slate-500">Saving results...</p>}
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            {questions.map((qItem, i) => {
              ...
              let bgClass = 'bg-surface-700 text-slate-400';
              if (isCorrect) bgClass = 'bg-green-500/30 text-green-300 border-green-500/50';
              else if (isWrong) bgClass = 'bg-red-500/30 text-red-300 border-red-500/50';
              else if (isCurrent) bgClass = 'bg-amber-500/30 text-amber-300 border-amber-500';
              else if (isAnswered) bgClass = 'bg-amber-500/20 text-amber-300 border-amber-500/30';

              return (
                <button key={qItem.id} onClick={() => setCurrentQ(i)}
                  className={`h-10 w-10 rounded-lg text-sm font-bold transition-all flex items-center justify-center border ${bgClass} ${isCurrent ? 'ring-2 ring-amber-400 ring-offset-2 ring-offset-surface-950' : ''}`}>
                  {i + 1}
                </button>
              );
            })}
        ...
        {/* Current Question */}
        <div className="glass-card rounded-2xl p-8 border border-surface-700 space-y-6">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <span className="amber-pill text-xs font-semibold">Question {currentQ + 1} of {questions.length}</span>
              <div className="h-2 flex-1 mx-4 bg-surface-700 rounded-full overflow-hidden">
                <div className="h-full bg-gradient-to-r from-amber-400 to-amber-600 transition-all duration-300" style={{ width: `${((currentQ + 1) / questions.length) * 100}%` }} />
              </div>
            </div>
          ...
          <div className="space-y-3">
            {(['a', 'b', 'c', 'd'] as const).map((key) => {
              ...
              let borderCl = 'border-surface-600 hover:border-surface-500';
              let bgCl = 'glass-card-hover';
              if (isCorrectOpt) { borderCl = 'border-green-500/50 bg-green-500/10'; bgCl = ''; }
              else if (isWrongOpt) { borderCl = 'border-red-500/50 bg-red-500/10'; bgCl = ''; }
              else if (isSelected) { borderCl = 'border-amber-500/60 bg-amber-500/15'; bgCl = ''; }

              return (
                <button key={key}
                  onClick={() => { if (!submitted) setAnswers((p) => ({ ...p, [q.id]: key })); }}
                  disabled={submitted}
                  className={`w-full ${bgCl} rounded-xl border-2 px-5 py-4 text-left transition-all cursor-pointer group ${borderCl} ${submitted ? 'cursor-not-allowed opacity-90' : ''} ${isCorrectOpt ? 'ring-2 ring-green-500/50' : ''} ${isWrongOpt ? 'ring-2 ring-red-500/50' : ''} ${isSelected && !submitted ? 'ring-2 ring-amber-500/50' : ''}`}>
                  <div className="flex items-start gap-4">
                    <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg font-bold text-sm transition-all ${
                      isCorrectOpt ? 'bg-green-500/30 text-green-300 border border-green-500/50' :
                      isWrongOpt ? 'bg-red-500/30 text-red-300 border border-red-500/50' :
                      isSelected ? 'bg-amber-500/30 text-amber-300 border border-amber-500/50' :
                      'bg-surface-700 text-slate-400 group-hover:bg-surface-600'
                    }`}>
                      {key.toUpperCase()}
                    </div>
                    <span className="flex-1 text-base font-medium text-white">{optText}</span>
                    {isCorrectOpt && <span className="text-green-400 text-lg">✓</span>}
                    {isWrongOpt && <span className="text-red-400 text-lg">✗</span>}
                  </div>
                </button>
              );
            })}
          ...
          {submitted && q.explanation && (
            <div className="mt-4 rounded-lg border border-blue-500/30 bg-blue-500/10 p-4">
              <p className="text-xs font-bold text-blue-400 uppercase tracking-wide mb-1">Explanation</p>
  ...
  return (
    <div className="space-y-8 pb-20 lg:pb-0">
      ...
      <div className="animate-fade-in-up rounded-2xl overflow-hidden">
        <div className="relative p-6 md:p-8 border border-amber-500/20" style={{ background: 'linear-gradient(135deg, #0a101f 0%, #141c2e 50%, #0a101f 100%)' }}>
          <div className="absolute inset-0 opacity-10">
            <div className="absolute top-0 right-0 w-40 h-40 bg-amber-400 rounded-full mix-blend-multiply filter blur-3xl" />
            <div className="absolute -bottom-8 left-20 w-40 h-40 bg-amber-500 rounded-full mix-blend-multiply filter blur-3xl" />
          </div>
          <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div className="space-y-2">
              <h2 className="text-xl md:text-2xl font-bold text-gradient">Active Mission</h2>
              <p className="text-slate-300">Complete 3 Chapter Tests Today</p>
            </div>
              ...
              <div className="flex-shrink-0">
                <div className="relative w-20 h-20 flex items-center justify-center">
                  ...
                  <div className="absolute flex flex-col items-center justify-center">
                    <span className="text-lg font-bold text-amber-400">{testsCompleted}/3</span>
                  </div>
                </div>
              </div>
              <div>
                <p className="text-sm font-medium text-slate-400">Tests Completed</p>
                <p className="text-2xl font-bold text-white">{3 - testsCompleted} more to go</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {!loadingGamification && gamification && (
        <div className="animate-fade-in-up grid gap-4 sm:grid-cols-3" style={{ animationDelay: '50ms' }}>
          {/* Daily Streak */}
          <div className="rounded-xl p-4 border border-amber-500/15 space-y-3" style={{ background: '#0a101f' }}>
            <div className="flex items-center justify-between">
          ...
          <div className="rounded-xl p-4 border border-amber-500/15 space-y-3" style={{ background: '#0a101f' }}>
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Points to Next Level</span>
            <div className="flex items-baseline gap-2">
              <span className="text-3xl font-bold text-white">{gamification.xp % 100}</span>
          ...
          <div className="rounded-xl p-4 border border-amber-500/15 space-y-3" style={{ background: '#0a101f' }}>
            ...
            <div className="text-3xl font-bold text-white">
              {gamification.level}
              <span className="text-sm text-slate-400 ml-1">Lvl</span>
            </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-1.5 rounded-full border border-amber-500/30 bg-amber-500/10 px-3 py-1">
              ...
              <span className="text-xs font-bold text-amber-400">AI-POWERED</span>
            </div>
          </div>
          <p className="text-slate-400 text-lg">Master your subjects with AI-generated practice tests</p>
        </div>
      </div>
      ...
      {assignedTests.length > 0 && (
        <section className="space-y-4 animate-fade-in-up" style={{ animationDelay: '150ms' }}>
          <div className="flex items-center gap-3">
            <h2 className="text-lg font-bold text-white">📋 Assigned by Teacher</h2>
            <span className="px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-400 text-xs font-bold border border-amber-500/30">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {assignedTests.map((test) => (
              <button
                ...
                className="glass-card-hover tilt-hover rounded-2xl p-6 border border-amber-500/20 text-left group overflow-hidden relative"
              >
                <div className="absolute inset-0 bg-gradient-to-br from-amber-500/0 to-amber-600/0 group-hover:from-amber-500/5 group-hover:to-amber-600/5 transition-all duration-300" />
                <div className="relative space-y-4">
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <span className="px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-400 text-xs font-bold">ASSIGNED</span>
                      <span className="px-2 py-0.5 rounded-full bg-surface-700 text-slate-400 text-xs font-medium capitalize">{test.difficulty}</span>
                    </div>
                    <h3 className="text-lg font-bold text-white group-hover:text-amber-100 transition-colors">{test.testName}</h3>
                    <p className="text-xs text-slate-400">{test.chapters?.join(', ')}</p>
                  </div>
                  <div className="flex items-center justify-between pt-2 border-t border-surface-700 group-hover:border-amber-500/30 transition-colors">
                    <p className="text-xs text-slate-400">{test.questionCount} Qs • {test.timeLimit}m limit</p>
                    <svg className="h-5 w-5 text-amber-400 transition-transform group-hover:translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {loadingAssigned && (
        <div className="glass-card py-6 flex items-center justify-center gap-3">
          <div className="h-5 w-5 animate-spin rounded-full border-2 border-surface-700 border-t-amber-400" />
          <p className="text-slate-400 text-sm">Checking for assigned tests...</p>
        </div>
      )}

      {/* CBSE Exam Format */}
      {board === 'CBSE' && (
        <div className="animate-fade-in-up glass-card rounded-2xl p-6 border border-surface-700" style={{ animationDelay: '200ms' }}>
          <div className="space-y-4">
            <h3 className="font-bold text-white text-lg flex items-center gap-2">
              <span className="text-amber-400">📋</span> CBSE Exam Format
            </h3>
            <div className="stagger-children grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
              {Object.entries(CBSE_SECTIONS).map(([key, sec]) => (
                <div key={key} className="stagger-item animate-scale-in glass-card tilt-hover rounded-lg p-3 border border-surface-700 space-y-1">
                  <p className="text-xs font-bold text-amber-400 uppercase tracking-wide">{sec.name}</p>
                  <p className="text-white font-bold text-2xl animate-count-up">{sec.count}</p>
                  <p className="text-slate-400 text-xs">{sec.marksEach} marks each</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full font-medium whitespace-nowrap transition-all duration-300 flex-shrink-0 ${
                selectedTopic === topic
                  ...
                  : 'bg-surface-800/80 text-slate-400 border border-surface-700/50 hover:border-surface-700'
              }`}
            >
      ...
      <div className="animate-fade-in flex gap-3" style={{ animationDelay: '250ms' }}>
        <h2 className="text-lg font-bold text-white self-center mr-2">📝 Practice Tests</h2>
        {subjects.map((s) => (
          <button key={s} onClick={() => setSelectedSubject(s)}
            className={`px-6 py-3 rounded-xl font-bold transition-all duration-300 ${selectedSubject === s ? 'btn-glow animate-pulse-glow' : 'btn-ghost border border-surface-700'}`}>
        ...
        {chapters.map((ch, idx) => {
          ...
          return (
            <button
              ...
              disabled={isLocked}
              className={`stagger-item animate-fade-in-up glass-card-hover ${!isLocked && 'tilt-hover'} rounded-2xl p-6 border transition-all duration-300 text-left group overflow-hidden relative ${
                isLocked
              ...
              {isLocked && (
                <div className="absolute inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center rounded-2xl">
                  <div className="text-center space-y-2">
                    ...
                    <p className="text-xs font-bold text-slate-300">Premium Chapter</p>
                  </div>
                </div>
              )}
              ...
              <div className="relative space-y-4">
                ...
                <div className="flex items-start justify-between">
                  <div className="space-y-2 flex-1">
                    ...
                    <h3 className={`text-lg font-bold transition-colors ${!isLocked ? 'text-white group-hover:text-amber-100' : 'text-slate-400'}`}>
                      {ch.name}
                    </h3>
                  </div>
                ...
                {/* Subtitle */}
                <div className="flex items-center justify-between pt-2 border-t border-surface-700 group-hover:border-amber-500/30 transition-colors">
                  <p className={`text-xs transition-colors ${!isLocked ? 'text-slate-400 group-hover:text-slate-300' : 'text-slate-500'}`}>
